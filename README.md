...
- [x] 本地端可编辑（默认只读，可在设置中开启；开启后方可"保存至本地"）

### 配置同步
- [x] **本地保存**：将编辑器左侧内容写入本地文件（需在设置中开启本地编辑权限）
- [x] **远端推送**：将编辑器右侧内容推送至远程服务器
- [x] **一键初始化**：远程无配置文件时，自动复制本地内容到右侧编辑器并推送
- [x] 推送前自动格式校验（JSON / YAML / TOML / INI / XML）
- [x] 推送时自动保留原换行符格式（LF / CRLF）
- [x] 单文件大小限制 5MB

### 生产环境保护
- [x] 生产环境安全锁（默认启用，可关闭；**仅对生产环境生效，开发环境不受限制**）
...
- [x] 备份列表浏览（按时间倒序）
- [x] 备份差异比对 — 备份（只读）↔ 远端（可编辑），支持修改后再覆盖
- [x] 覆盖远端：用右侧编辑器当前内容推送至远端
- [x] 删除指定备份
...
│   │
│   ├── hooks/
│   │   ├── useAppData.ts            # 数据持久化与加载
│   │   ├── useBackupOperations.ts   # 备份管理（列表、比对、恢复、删除）
│   │   ├── useEditorSession.ts      # 编辑器会话与配置同步
│   │   └── useProductionLock.ts     # 生产环境安全锁
│   │
│   ├── components/
